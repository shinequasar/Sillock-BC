pragma solidity 0.7.4; // TODO : 0.5.6 stable for klaytn

import "./ERC721/ERC721.sol";
// TODO: import and change contract name before migration
// import "./ERC721/ERC721Enumerable.sol";
// contract CertToken is ERC721, ERC721Enumerable {
...
contract CertToken is ERC721{
    ...
    uint256 private issueFee = 1 ether; // TODO: KLAY!

    event NewCert(uint256 indexed certId, string name, int no, address url, address holder, string issuer, bool exchangable, bool authorized, uint date, uint now);    
    ...
    function mintCard(string memory _name, address _holder, string memory _url, bool _exchangable, uint _date) haveEmptyCert public payable {
        ...
        // emit NewCert(tokenId, _name, certNo, _url, _holder, issuer, _exchangable, authorized, now);
    }
    ...
    */
    ...
    function mintEmptyCard(address _issuer, uint256 _num) onlyOwner public payable{
        // 발행자의 주소를 받고 해당 주소와 매핑된 갯수를 한개 추가함
        // TODO : mapping시 처음 인자가 0으로 초기화되는지 검사
        _address2EmptyCert[_issuer].add(_num); // using SafeMath
    }

    function useEmptyCert(address _issuer, uint256 _num) onlyOwner public{
        _address2EmptyCert[_issuer].sub(_num); // Using SafeMath
    }
    ...
    modifier haveEmptyCert {
        require ((msg.sender == owner) || (_address2EmptyCert[msg.sender] > 0), "You don't have enough Empty Certs.");
        _;
    }
